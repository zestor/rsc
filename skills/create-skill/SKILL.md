...
- `description`: 1-1024 characters describing what the skill does and when to use it:
  ...
  - Include keywords and trigger phrases that help identify when it should be used (important for agent discovery)
  - Descriptions may contain `:`, `#`, or other special characters that must be escaped via quotes. **Always** wrap in double quotes to be safe.
...
2. **Choose a name**: Pick a descriptive name following the requirements above.

3. **Write a clear description**: This is crucial for skill discovery. Follow the requirements above.
...
6. **Validate the skill**: After creating all relevant files, validate using the skills-ref library. Do NOT validate until you have finished preparing all files. Pass the skill **directory** (not a single file or ZIP) to the validate command:
   ...
   ```bash
   agentskills validate {skills_dir}/{skill-name}/
   ```

   If validation fails, read the error message and fix before proceeding.

7. **Prepare for sharing**: A skill must be shared as a single zip or md file — never as a directory path. List the contents of the skill directory first to understand skill contents.
   - If the skill directory contains a SKILL.md only and has no other bundled resources, share the SKILL.md file directly
   - If the skill has any bundled resources (scripts, references, assets): zip the entire skill directory before sharing. Do NOT extract just the SKILL.md — the bundled files are part of the skill. Use `.zip` format, not `.tar` or `.tar.gz`
   - The packaged skill must contain no more than **100 files** and be no larger than **70 MB**.
...
**Error: "invalid YAML in frontmatter" (e.g., "mapping values are not allowed in this context")**

The `description` value contains a character YAML treats specially — most commonly `:`, but also `#`, `[`, `{`, a leading `-`, or a reserved word like `yes`/`no`/`null`. Wrap the description in double quotes so YAML reads it as a literal string.

- Wrong: `description: Use when: the user asks X`
- Correct: `description: "Use when: the user asks X"`

Quoting is always safe, even when not strictly required — when in doubt, quote.
...
- Must match the directory name exactly
- Must not duplicate an existing skill name of the same scope (user custom skill). Check existing skills before deciding on a name for any new skill.
